...
class PythonMatrixService {
  ...
  private startPythonProcess(): void {
    this.process = spawn('python3', ['./scripts/matrix.py']);
    ...
    this.process.stderr?.on('data', (data: Buffer) => {
      console.error(`Python matrix service error: ${data.toString()}`);
      if (this.requestQueue.length > 0) {
    ...
    this.process.on('close', () => {
      console.log('Python matrix service closed, restarting...');
  ...
  public evaluateMatrix(expression: string): Promise<string> {
...
export const pythonMatrixService = new PythonMatrixService();